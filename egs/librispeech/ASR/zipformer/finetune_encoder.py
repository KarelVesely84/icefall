#!/usr/bin/env python3
# Copyright    2023-2025  Brno University of Technology (author: Karel Vesely)
...
"""
Re-train the model on new language. The encoder is ported,
the predictor, joiner and CTC head are randomly initialized.

The learning rate scheduling is redesigned.
- The `encoder` learning rate has a slow warm-up (`--encoder-lr-warmup-batches`).
- For `encoder` and `encoder_embed` modules, the `batch_counts` start from 100k,
  so the encoder bypasses are not reset.
- The rest of the model is starting from 0 updates (predictor, joiner, ctc-head).

Usage:

export CUDA_VISIBLE_DEVICES="0,1,2,3"

./zipformer/finetune_encoder.py \
  --world-size 4 \
  --num-epochs 30 \
  --start-epoch 1 \
  --use-fp16 1 \
  --exp-dir zipformer/exp_finetune_encoder \
  --bpe-model data/lang_bpe_500/bpe.model \
  --max-duration 200 \
  --lr-scheduler Eden2 \
  --lr-batches 20000 \
  --initial-encoder <path_to_pt_file> \
  --train-manifests <cutset_files> \
  --validation-manifestes <cutset_files>

Make sure that the model-related arguments are the same as in pre-trainining.
...
from lhotse.caching import set_caching_enabled
from lhotse.cut import Cut, CutSet
from lhotse.dataset import SpecAugment
from lhotse.dataset.sampling.base import CutSampler
from lhotse.utils import fix_random_seed
from model import AsrModel
from optim import Eden, Eden2, ScaledAdam
...
def set_batch_count(model: Union[nn.Module, DDP], batch_count: float) -> None:
    ...
    prefixes = set(['encoder_embed', 'encoder'])

    for name, module in model.named_modules():
        if hasattr(module, "batch_count"):
            prefix = name.split('.')[0]
            if prefix in prefixes:
                # logging.info(f"{name}, +100k steps")
                # add extra 100k steps (pre-trained model)
                module.batch_count = batch_count + 100000
            else:
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=80,
        help="Feature dim on input of `encoder_embed` (number of FBANK filters)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="""Load initial encoder parameters. The rest of the model stays
        randomly initialized (model = encoder + decoder + joiner + CTC head).
        This pt_file contains full ASR model, from which `encoder` and
        `encoder_embed` parts are ported.""",
    )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default="Eden",
        choices=["Eden", "Eden2"],
        help="""Select the learning rate scheduler. Eden is based on number of steps
        and epoch. Eden2 is purely based on number of steps.""",
    )
    ...
    parser.add_argument(
        "--encoder-lr-warmup-batches",
        type=float,
        default=3000,
        help="""Warmup the `encoder.lr_scale` linearly for initial N batches.""",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=40.0,
        help="Maximal duration of an utterance in seconds, used in training/validation cut-set filtering.",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        nargs='+',
        help="Train manifests (e.g. 'data/fbank/imported_cuts_train.jsonl.gz [...]')",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        nargs='+',
        help="Validation manifests (e.g. 'data/fbank/imported_cuts_dev.jsonl.gz [...]')",
    )
...
def load_model_params(
    ...
    Args:
        ...
        init_modules (list[str]): List of module prefixes, selects modules
            to be overwritten by `ckpt` parameters.

    """
    logging.info(f"Loading checkpoint from `{ckpt}`")
    torch.serialization.add_safe_globals([PosixPath])  # so `weights_only=True` works
    ...
    if not init_modules:
        ...
        # `dst_state_dict` is a mix of parameters from `model` and `checkpoint`,
        # `checkpoint` version is selected according to prefixes in `init_modules`:
        model.load_state_dict(dst_state_dict, strict=strict)
...
def get_feature_lens(supervisions: dict) -> Tensor:
    """
    Get feature lens, being part of a batch.

    Note:
    There can be concatenated cuts, having (N > 1) supervisions for 1 cut.
    (see `--concatenate-cuts` in `asr_datamodule.py`)
    """

    num_cuts = torch.unique(supervisions["sequence_idx"]).shape[0]
    num_supervisions = supervisions["sequence_idx"].shape[0]

    if num_cuts == num_supervisions:
        return supervisions["num_frames"]
    else:
        # compute feature_lens when `CutConcatenate` was used
        feature_lens = []
        for ii in range(num_cuts):
            mask = supervisions["sequence_idx"] == ii
            end_frames = supervisions["start_frame"][mask] + supervisions["num_frames"][mask]
            feature_lens.append(max(end_frames))

        return default_collate(feature_lens)


def get_texts(supervisions: dict) -> list[str]:
    """
    Get texts, map them to feature matrices in a batch.
    ...
    if num_cuts == num_supervisions:
        return supervisions["text"]
    else:
        # concatenate transcripts when `CutConcatenate` was used
        texts = []
        for ii in range(num_cuts):
            mask = supervisions["sequence_idx"] == ii
            selected_text = [txt for ii, txt in enumerate(supervisions["text"]) if mask[ii]]
            texts.append(" ".join(selected_text))
        return texts
...
def compute_loss(
    ...
    feature_lens = get_feature_lens(supervisions).to(device)

    texts = get_texts(supervisions)
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        # extra warm-up for encoder (0.0...1.0, linear for `encoder_lr_warmup_batches` steps)
        if params.encoder_lr_warmup_batches is not None:
            if isinstance(model, DDP):
                model_ref = model.module
            else:
                model_ref = model
            warmup_factor = params.batch_idx_train / params.encoder_lr_warmup_batches
            warmup_factor = max(warmup_factor, 0.001)  # at least 0.001
            warmup_factor = min(warmup_factor, 1.0)    # not more than 1.0
            model_ref.encoder_embed.lr_scale = warmup_factor
            model_ref.encoder.lr_scale = warmup_factor
...
def run(rank, world_size, args):
    ...
    if world_size > 1:
        setup_dist(rank, world_size, params.master_port)  # cuda.set_device()
    ...
    if args.initial_encoder is not None and args.start_epoch == 1:
        # Just load the encoder:
        logging.info(f"Loading `initial_encoder` {args.initial_encoder} (external checkpoint)")

        # get module names from pre-encoder and encoder
        module_name_prefixes = [ "encoder_embed", "encoder" ]
        load_model_params(args.initial_encoder, model, module_name_prefixes, strict=True)
    ...
    optimizer = ScaledAdam(
        ...
        clipping_scale=2.0,  # note: gradient clipping already inside ScaledAdam
    )

    # LR SCHEDULING METHOD
    if params.lr_scheduler == "Eden":
        scheduler = Eden(optimizer, params.lr_batches, params.lr_epochs)
    elif params.lr_scheduler == "Eden2":
    ...
    for manifest in args.train_manifests:
        train_cuts += asr_datamodule.load_manifest(manifest)


    def remove_short_and_long_utt(c: Cut):
        # Keep only utterances with duration between 0.5 second and 40 seconds
        #
        # Caution: There is a reason to select 40.0 here. Please see
        # ../local/display_manifest_statistics.py
        #
        # You should use ../local/display_manifest_statistics.py to get
        # an utterance duration distribution for your dataset to select
        # the threshold
        if c.duration < 0.5 or c.duration > params.max_utt_duration:
            logging.warning(
    ...
    logging.info(f"Training data filtering, max_utt_duration {params.max_utt_duration}")
    ...
    # shuffle the cuts (we mix the merged databases)
    # - TODO: is the disk server stressed ? (we could reshuffle features to /tmp to speed up)
    train_cuts = train_cuts.to_eager() # make sure CutSet had lazy evaluation disabled !
    logging.info(f"Train cuts should not be lazy for efficient shuffling: train_cuts.is_lazy = {train_cuts.is_lazy}")
    train_cuts_shuffled = train_cuts.shuffle(rng=random.Random(42), buffer_size=10e8)

    train_dl = asr_datamodule.train_dataloaders(
        train_cuts_shuffled, sampler_state_dict=sampler_state_dict
    )

    # load validation manifests
    valid_cuts = CutSet()
    for manifest in args.validation_manifests:
        valid_cuts += asr_datamodule.load_manifest(manifest)
    ...
    for epoch in range(params.start_epoch, params.num_epochs + 1):
        ...
        # release all unused cached memory from PyTorch
        # (free memory will be visible in nvidia-smi)
        if torch.cuda.is_available():
